 ...
 * @return WP_User|null
 */
function auth0_wp_test_hook_auth0_get_wp_user( $user, $userinfo ) {
	$found_user = get_user_by( 'email', $userinfo->email );
	$user       = $found_user instanceof WP_User ? $user : null;
	return $user;
}
// add_filter( 'auth0_get_wp_user', 'auth0_wp_test_hook_auth0_get_wp_user', 1, 2 );
 ...
 * @return string
 */
function auth0_wp_test_hook_auth0_verify_email_page( $html, $userinfo, $id_token ) {
	$html = 'Hi ' . $userinfo->email . '!<br>' . $html;
	$html = str_replace( 'email', 'banana', $html );
	return $html;
}
// add_filter( 'auth0_verify_email_page', 'auth0_wp_test_hook_auth0_verify_email_page', 10, 3 );
 ...
 * @return string mixed
 */
function auth0_wp_test_hook_auth0_get_auto_login_connection( $connection ) {
	return ! empty( $_GET['connection'] ) ? rawurlencode( $_GET['connection'] ) : $connection;
}
// add_filter( 'auth0_get_auto_login_connection', 'auth0_wp_test_hook_auth0_get_auto_login_connection' );
 ...
 * @return mixed
 */
function auth0_wp_test_hook_wp_auth0_get_option( $value, $key ) {
	$value = 'bad_key' === $key ? 'That is a bad key and you know it' : $value;
	return $value;
}
// add_filter( 'wp_auth0_get_option', 'auth0_wp_test_hook_wp_auth0_get_option', 10, 2 );
 ...
 * @return WP_User
 */
function auth0_wp_test_hook_auth0_migration_ws_authenticated( $user ) {
	$user->data->display_name = 'Sir ' . $user->data->display_name . ', Esquire';
	return $user;
}
// add_filter( 'auth0_migration_ws_authenticated', 'auth0_wp_test_hook_auth0_migration_ws_authenticated' );
 ...
 * @return bool
 */
function auth0_wp_test_hook_wpa0_should_create_user( $should_create, $userinfo ) {
	$should_create = false === strpos( 'josh', $userinfo->email );
	return $should_create;
}
// add_filter( 'wpa0_should_create_user', 'auth0_wp_test_hook_wpa0_should_create_user' );
 ...
 * @return string
 */
function auth0_wp_test_hook_auth0_login_css( $css ) {
	$css .= '
...
// add_filter( 'auth0_login_css', 'auth0_wp_test_hook_auth0_login_css' );
 ...
 */
function auth0_wp_test_hook_auth0_settings_fields( $options, $id ) {
	switch ( $id ) {
		case 'basic':
			$options[] = array(
				...
				'function' => 'auth0_wp_test_render_custom_basic_opt_name',
			);
			break;
		case 'features':
			break;
...
// add_filter( 'auth0_settings_fields', 'auth0_wp_test_hook_auth0_settings_fields', 10, 2 );
 ...
 *
 * @see auth0_wp_test_hook_auth0_settings_fields()
 */
function auth0_wp_test_render_custom_basic_opt_name( $args ) {
 ...
 * @return array
 */
function auth0_wp_test_hook_auth0_auth_scope( $scopes ) {
	// Add offline_access to include a refresh token.
	// See auth0_wp_test_hook_auth0_user_login() for how this token can be used.
	$scopes[] = 'offline_access';
	return $scopes;
}
// add_filter( 'auth0_auth_scope', 'auth0_wp_test_hook_auth0_auth_scope' );
 ...
 * @return string
 */
function auth0_wp_test_hook_prefix_cookie_name( $cookie_name ) {
	return 'STYXKEY_' . $cookie_name;
}
// add_filter( 'auth0_state_cookie_name', 'auth0_wp_test_hook_prefix_cookie_name' );
// add_filter( 'auth0_nonce_cookie_name', 'auth0_wp_test_hook_prefix_cookie_name' );
 ...
 * @return string
 */
function auth0_wp_test_hook_settings_constant_prefix( $prefix ) {
	// Replace the prefix with something else.
	// return 'AUTH_ENV_';
	// Prefix the prefix.
	return 'PREFIX_' . $prefix;
}
// add_filter( 'auth0_settings_constant_prefix', 'auth0_wp_test_hook_settings_constant_prefix' );
 ...
 * @return mixed
 */
function auth0_wp_test_hook_authorize_url_params( $params, $connection, $redirect_to ) {
 ...
 * @return string
 */
function auth0_wp_test_hook_authorize_url( $auth_url, $auth_params ) {
...
// add_filter( 'auth0_authorize_url', 'auth0_wp_test_hook_authorize_url', 10, 2 );
 ...
 * @return string
 */
function auth0_wp_test_hook_die_on_login_output( $html, $msg, $code, $login_link ) {
	return sprintf(
...
//add_filter( 'auth0_die_on_login_output', 'auth0_wp_test_hook_die_on_login_output', 10, 4 );
